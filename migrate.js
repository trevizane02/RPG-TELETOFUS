...
import { ITEM_SEEDS } from "./data/items.js";
    ...
    try {
      ...
      await client.query(`
        ...
        BEGIN
          ...
          END IF;
          IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='items' AND column_name='class_req') THEN
            ALTER TABLE items ADD COLUMN class_req TEXT;
          END IF;
          IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='items' AND column_name='level_req') THEN
            ALTER TABLE items ADD COLUMN level_req INT;
          END IF;
      ...
      for (const it of ITEM_SEEDS) {
        await client.query(
          `
          INSERT INTO items (key, name, slot, rarity, drop_rate, map_key, class_req, level_req, atk_min, atk_max, def_min, def_max, hp_min, hp_max, crit_min, crit_max, boss_only, boss_dungeon_only)
